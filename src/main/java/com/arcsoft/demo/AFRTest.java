...
public class AFRTest {
    ...
    public static void main(String[] args) {
        ...
        if (bUseRAWFile) {
            ...
            String filePathA = "1.jpg";
            String filePathB = "2.jpg";

            inputImgA = loadImage(filePathA);
            inputImgB = loadImage(filePathB);
        }

        System.out.println(String.format("两图相似度 is %f", compareFaceSimilarity(hFDEngine, hFREngine, inputImgA, inputImgB)));
    ...
    public static float compareFaceSimilarity(Pointer hFDEngine, Pointer hFREngine, ASVLOFFSCREEN inputImgA, ASVLOFFSCREEN inputImgB) {
        ...
        try {
            ...
            fileHelper.createFile("img1.txt", faceFeatureA.toByteArray());
        } catch (IOException e) {
            e.printStackTrace();
        } catch (Exception e) {
            e.printStackTrace();
